...
void pushContextAbsoluteClock(std::function<void()> callbackFunction, bool drawDefaultUI) {
  // NOTE: duplicated code from pushContext()
  // Create a new context and push it on to the stack
  ImGuiContext* newContext = ImGui::CreateContext(render::engine->getImGuiGlobalFontAtlas());
  ImGuiIO& oldIO = ImGui::GetIO(); // used to GLFW + OpenGL data to the new IO object
#ifdef IMGUI_HAS_DOCK
  ...
  // Record the time when we start frame 0
  frameZeroTime = std::chrono::high_resolution_clock::now();

  while (contextStack.size() >= currentContextStackSize) {
    // Determine whether this frame should be even or odd, based on absolute time
    // since frame 0 (and also based on options::drawEvenFrameFirst)
    ...
    if (!options::drawEvenFrameFirst) state::isEvenFrame = !state::isEvenFrame;
...
void renderSceneToScreen(bool flatLighting) {
  ...
  if (options::debugDrawPickBuffer) {
    ...
    render::engine->applyLightingTransform(render::engine->sceneColorFinal, flatLighting);
  }
}
...
void draw(bool withUI, bool withContextCallback, bool flatLighting) {
  ...
  renderSceneToScreen(flatLighting);
...
void show(size_t forFrames) {
  ...
  if (options::renderEvenOddAbsoluteClock) {
    pushContextAbsoluteClock(checkFrames);
  } else {
